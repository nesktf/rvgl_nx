...
void so_finalize(so_module *mod) {
  ...
  debugPrintf("%s: so_finalize starting, size=%u KB\n", mod->name, (u32)(mod->load_size / 1024));
  ...
  debugPrintf("%s: svcMapProcessCodeMemory(virtbase=%p, base=%p, size=0x%lx) -> 0x%08x\n",
              mod->name, mod->load_virtbase, mod->load_base, (unsigned long)mod->load_size, rc);
  if (R_FAILED(rc)) fatal_error("Error: svcMapProcessCodeMemory failed on %s:\n%08x", mod->name, rc);
  ...
  for (int want_x = 1; want_x >= 0; want_x--) {
    ...
    while (pg < num_pages) {
      ...
      debugPrintf("%s: svcSetProcessMemoryPermission(%p, 0x%lx, %s) -> 0x%08x\n",
                  mod->name, (void *)addr, (unsigned long)size, want_x ? "RX" : "RW", rc);
  ...
  // Rebase syms and dynstrtab to load_virtbase now that load_base is unmapped
  if (mod->syms)
    mod->syms = (Elf64_Sym *)((uintptr_t)mod->load_virtbase + ((uintptr_t)mod->syms - (uintptr_t)mod->load_base));
  if (mod->dynstrtab)
    mod->dynstrtab = (char *)((uintptr_t)mod->load_virtbase + ((uintptr_t)mod->dynstrtab - (uintptr_t)mod->load_base));

  debugPrintf("%s: so_finalize done\n", mod->name);
}
...
void so_execute_init_array(so_module *mod) {
  for (int i = 0; i < mod->elf_hdr->e_shnum; i++) {
    ...
    if (strcmp(sh_name, ".init_array") == 0) {
      ...
      int count = mod->sec_hdr[i].sh_size / 8;
      debugPrintf("%s: executing %d init_array entries at %p\n", mod->name, count, init_array);
      for (int j = 0; j < count; j++) {
        if (init_array[j] != 0) {
          debugPrintf("%s: calling init_array[%d] (%p)...\n", mod->name, j, init_array[j]);
          init_array[j]();
          debugPrintf("%s: init_array[%d] returned\n", mod->name, j);
        }
      }
    }
  }
